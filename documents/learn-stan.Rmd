...
As such, whiel Stan won't force you to do it, you should always take the time to think carefully about your priors and test the affect of your choices on your model outcomes. 
...
initials <-
  list(
    ...
    sigma_observation = .2,
    sigma_harvest = 0.2,
    u = (hake_data$harvest / exp(7.973823))

    # iu = 1 / (hake_data$harvest / exp(7.973823))
  )
...
ss_hake_fit <- stan(
  file = here::here("scripts","ss_schaefer.stan"),
  data = hake_data,
  chains = n_chains,
  warmup = warmups,
  iter = total_iterations,
...
```{r ss-hake-diagnostics}


ss_hake_diagnostics <- rstan::get_sampler_params(ss_hake_fit) %>% 
  set_names(1:n_chains) %>% 
   map_df(as_data_frame,.id = 'chain') %>% 
  group_by(chain) %>% 
  mutate(iteration = 1:length(chain)) %>% 
...
```{r ss-hake-plot}
ss_hake_mcmc <- ss_hake_fit %>%
  rstan::extract(permuted = F)

hake_names <- dimnames(hake_mcmc)$parameters

ss_hake_mcmc <- plyr::alply(ss_hake_mcmc, .margins = 3, .dims = T) %>%
  map_df(as_data_frame, .id = 'variable') %>%
  gather(chain, estimate, -variable) %>%
  mutate(chain = str_replace_all(chain,'\\D',"") %>% as.factor()) %>%
  group_by(variable, chain) %>%
...
ss_hake_summary <- summary(ss_hake_fit)$summary %>% 
  as.data.frame() %>% 
  mutate(variable = rownames(.)) %>% 
  as_data_frame()
...
## Plot SS Hake Results

```{r plot-ss-hake}


ss_hake_fit_index <- ss_hake_summary %>% 
  filter(str_detect(variable,'index')) %>% 
  mutate(year = str_replace_all(variable,'\\D','') %>% as.numeric()) %>% 
  mutate(variable = str_replace_all(variable,"(\\d)|(\\[)|(\\])","")) %>% 
  select(mean, variable, year) %>% 
...
biomass <- ss_hake_summary %>% 
  filter(str_detect(variable,'population')) %>% 
  mutate(year = str_replace_all(variable,'\\D','') %>% as.numeric()) %>% mutate(year = hake$year)


harvest <- ss_hake_summary %>% 
  filter(str_detect(variable,'harvest_hat')) %>% 
  mutate(year = str_replace_all(variable,'\\D','') %>% as.numeric()) %>% mutate(year = hake$year)
